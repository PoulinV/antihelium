...
*/
  ...
  else if(A_nuclei == 2)p_to_pbar_file = fopen(FILE_NAME_DE_H_ON_H,"w");
  else if(A_nuclei == 3)p_to_pbar_file = fopen(FILE_NAME_HE3_H_ON_H,"w");
  else if(A_nuclei == 4)p_to_pbar_file = fopen(FILE_NAME_HE4_H_ON_H,"w");
...
*/
  ...
  else if(A_nuclei == 4)p_to_pbar_file = fopen(FILE_NAME_HE4_H_ON_H,"r");
  for (i_proton=0;i_proton<=DIM_TAB_PROTON;i_proton++)
  {
    for (i_pbar=0;i_pbar<=DIM_TAB_PBAR;i_pbar++)
    {
...
else if(A_nuclei == 4)p_to_pbar_file = fopen(FILE_NAME_HE4_H_ON_HE,"r");
for (i_proton=0;i_proton<=DIM_TAB_PROTON;i_proton++)
  {
    for (i_pbar=0;i_pbar<=DIM_TAB_PBAR;i_pbar++)
    {
...
else if(A_nuclei == 4)p_to_pbar_file = fopen(FILE_NAME_HE4_HE_ON_H,"r");
...
else if(A_nuclei == 4)p_to_pbar_file = fopen(FILE_NAME_HE4_HE_ON_HE,"r");
...
*/
  ...
  for (i_nucleon=0;i_nucleon<=DIM_TAB_PROTON;i_nucleon++)
  {
    ...
    for (i_pbar=0;i_pbar<=DIM_TAB_PBAR;i_pbar++)
    {
      ...
      pow((T_PBAR_MAX/T_PBAR_MIN*A_nuclei),((double)i_pbar/(double)DIM_TAB_PBAR));
      E_pbar = T_pbar + MASSE_PROTON;
      pt_Cross_Section->DSPBAR_SUR_DEPBAR_HE_ON_HE[i_pbar][i_nucleon] = pow((4.0),(2.2/3.0)) *
...
/*VP
*/
double sigma_total_pH_MDGS_s(double s)
{
	double Zpp  = 33.44;
	double M    = 2.06;
	double Bpp  = PI * pow((2.99792458e10*6.58211915e-25/M),2.) / mb_cm2;
...
/*
*/
double sigma_elastic_pH_MDGS_s(double s)
{
	double Zpp  = 144.98;
	double M    = 3.06;
	double Bpp  = PI * pow((2.99792458e10*6.58211915e-25/M),2.) / mb_cm2;
...
/*
*/
double sigma_inelastic_pH_MDGS_s(double s)
{
	return (sigma_total_pH_MDGS_s(s) - sigma_elastic_pH_MDGS_s(s));
}
/********************************************************************************************/
/*
...
/********************************************************************************************/
/* Same as above directly express in terms of CMF quantities
*/
double invariant_multiplicity_pH_apX_MDGS_F12_CMF(double xr,double ptstar,double s)
{
  double phi, sig;
	double C1 = 4.499;
	double C2 = 3.41;
	...
	if(xr<1.0)
	{
		phi = C3 * pow(sqrt(s),C4) * exp(-C5*ptstar)  +  C6 * pow(sqrt(s),C7) * exp(-C8*pow(ptstar,2.));
		sig = phi * pow((1.0-xr),C1) * exp(-C2*xr);
	}
	else sig=0;
	return sig;
}
...
/********************************************************************************************/
/********************************************************************************************/
double E_d3S_on_dP3_PBAR_ECM(double E_CMF,double pLstar,double pTstar)
{
  double Q,Estar,E_MAX_star,XR,f_XR,A,B;
  double XR_min,DXR,LE_sur_RS;
  double resultat;
...
*/
  ...
  A = 3.95 * exp(-2.76*XR);
  B = 40.5 * exp(-3.21*XR) * pow(XR,2.13);
  resultat = f_XR * exp(-A*pTstar-B*pTstar*pTstar);
...
/***********************/
double E_d3S_on_dP3_DBAR_ECM(double E_CMF,double pLstar,double pTstar)
{
  double Q,Estar,E_MAX_star,p1Lstar,p2Lstar,p1Tstar,p2Tstar,E1star,E_available;
  double resultat;
//ddd
  Q = E_CMF - 6.*MASSE_PROTON;
  E_MAX_star = (pow(E_CMF,2) - pow(4.*MASSE_PROTON,2) + pow(M_NUCLEI,2)) /
  (2.*E_CMF);
  Estar = sqrt(pow(M_NUCLEI,2) + pow(pLstar,2) + pow(pTstar,2));
/*
* SOME CHECKS ARE IN ORDER.
*/
  resultat = 0.0;
  if (Q<=0.0) goto LA_FIN;
  if (Estar>=E_MAX_star) goto LA_FIN;

  p1Lstar = p2Lstar = pLstar/A_NUCLEI;
  p1Tstar = p2Tstar = pTstar/A_NUCLEI;
  E1star = sqrt(pow(MASSE_PROTON,2) + pow(p1Lstar,2) + pow(p1Tstar,2));
/*
* We have a variety of choices for the remaining energy.
* E_available = E_CMF - 2.*E1star;
* E_available = E_CMF - E1star - MASSE_PROTON;
* See also :
*/
  E_available = E_CMF - 2.*E1star;
  double E_available_2 = E_CMF - 4.*E1star;
  double P_coal = 0.248;
  double SIGMA_INELASTIC_P_H=40.0e-27;
  if(A_NUCLEI == 2)resultat = (2./MASSE_PROTON) * (4.*PI*pow(P_coal,3)/8/3.) *
  1.3*(E_d3S_on_dP3_PBAR_ECM(E_CMF,p1Lstar,p1Tstar) *
  E_d3S_on_dP3_PBAR_ECM(E_available,p2Lstar,p2Tstar)) *
  (1.e-27*1.e-27/SIGMA_INELASTIC_P_H);
  else if(A_NUCLEI == 3)resultat = (2./MASSE_PROTON) * pow((4.*PI*pow(P_coal,3)/8/3.),2) * //VP added a factor 1/8 as advocated by Korsmeier et al.
  E_d3S_on_dP3_PBAR_ECM(E_CMF,p1Lstar,p1Tstar) *1.e-27*
  1.3*E_d3S_on_dP3_PBAR_ECM(E_available,p2Lstar,p2Tstar)*1.e-27 *
  1.3*E_d3S_on_dP3_PBAR_ECM(E_available_2,p2Lstar,p2Tstar) *
  (1.e-27/pow(SIGMA_INELASTIC_P_H,2));
LA_FIN :
  return resultat;
}

/********************************************************************************************/
double E_d3S_on_dP3_DBAR_LAB(double Ep,double pL,double pT,
double *E_CMF,double *pLstar,double *pTstar)
{
  double gamma,beta,EDbar,resultat;

  gamma = sqrt((Ep+MASSE_PROTON)/(2.*MASSE_PROTON));
  beta  = sqrt((Ep-MASSE_PROTON)/(Ep+MASSE_PROTON));
  EDbar = sqrt(pow(M_NUCLEI,2) + pow(pL,2) + pow(pT,2));
  *E_CMF = sqrt(2.*MASSE_PROTON*(Ep+MASSE_PROTON));
  *pLstar = gamma * (pL - beta*EDbar);
  *pTstar = pT;

  resultat = E_d3S_on_dP3_DBAR_ECM(*E_CMF,*pLstar,*pTstar);
...
double E_d3S_on_d3P_3HE_H_ON_H_LAB_MDGS_F12_coalescence(double E_proton,double pL,double pT, double P_coal, double A_nuclei, double Z_nuclei){
  ...
  double P_proton,P_proton_CMF,P_proton_available,P_proton_available_2,E_pbar,E_available,E_available_2,mt_pbar,y_pbar,resultat,Ed3S_on_d3P_pbar,Ed3S_on_d3P_nbar,B,E_CMF_SEUIL_PROTON;
  double E_CMF = sqrt(2.*MASSE_PROTON*(E_proton+MASSE_PROTON));
  double s = E_CMF*E_CMF;
  double E_MAX_star = (pow(E_CMF,2) - pow((2+A_nuclei)*MASSE_PROTON,2) + pow(M_NUCLEI,2)) / (2.*E_CMF);
  double xr;
  double E_CMF_SEUIL = sqrt(2.*MASSE_PROTON*(E_PROTON_MIN+MASSE_PROTON));
  double Ed3S_on_d3P_at_E_CMF=0,Ed3S_on_d3P_at_E_available=0,Ed3S_on_d3P_at_E_available_2=0;
  P_proton_CMF = sqrt(pow(E_CMF,2) - pow(MASSE_PROTON,2));
  // printf("P_proton_CMF %e \n",P_proton_CMF );
  P_proton = sqrt(pow(E_proton,2) - pow(MASSE_PROTON,2));

  double p1Lstar,p2Lstar,p1Tstar,p2Tstar,E1star;
  double gamma = sqrt((E_proton+MASSE_PROTON)/(2.*MASSE_PROTON));
  double beta  = sqrt((E_proton-MASSE_PROTON)/(E_proton+MASSE_PROTON));
  double E_product = sqrt(pow(M_NUCLEI,2) + pow(pL,2) + pow(pT,2));
  double pLstar = gamma * (pL - beta*E_product);
  double pTstar = pT;
  double Estar = sqrt(pow(M_NUCLEI,2) + pow(pLstar,2) + pow(pTstar,2));

  // printf("P_proton%e P_proton_CMF*gamma %e\n",P_proton,gamma*(P_proton_CMF+beta*E_CMF)/2.042);

/*
* SOME CHECKS ARE IN ORDER.
*/
  resultat = 0.0;
  // printf("E_CMF_SEUIL %e E_PROTON_MIN %e\n", E_CMF_SEUIL,E_PROTON_MIN);
  if (E_CMF<=E_CMF_SEUIL) return resultat;
  if (Estar>=E_MAX_star) return resultat;
  // printf("E_MAX_star %e E_star %e\n", E_MAX_star,Estar);
  p1Lstar = p2Lstar = pLstar/A_nuclei;
  p1Tstar = p2Tstar = pTstar/A_nuclei;
  E1star = sqrt(pow(MASSE_PROTON,2) + pow(p1Lstar,2) + pow(p1Tstar,2));
/*
...
*/
  ...
  B  = M_NUCLEI/(pow(MASSE_PROTON,Z_nuclei)*pow(MASS_NEUTRON*1e-3,A_nuclei-Z_nuclei))*pow(4*PI/3*pow(P_coal,3)/8,A_nuclei-1); //GeV^4

  // printf("A_nuclei %e Z_nuclei %e B %e\n",A_nuclei,Z_nuclei, B);
  B *= sigma_inelastic_pH_MDGS(E_proton);

//B  = BA_COAL_FID; //If we use directly the measured value by collaborations like ALICE.
//BEWARE HERE : antiHe3 and antiH3 are produced with the same coalescence factor B3.

//B *= pow(1.3,A_nuclei-Z_nuclei);//Antineutron over-production due to isospin symmetry breaking. But this DOES NOT SEEM to have been taken into account by ALICE.

//One needs to switch from LI differential multiplicities to LI differential cross sections.
//B /= pow(sigma_total_pH_MDGS(E_proton)    ,A_nuclei-1); //GeV^4 This prescription is used in Korsmeier et al.

/*** Computation as advocated by Chardonnet et al; astro-ph/9005110 */

  resultat = B;
  int i;
  for(i=0;i<A_nuclei;i++){

    E_MAX_star = (pow(E_CMF,2) - pow(3*MASSE_PROTON,2) + pow(MASSE_PROTON,2)) / (2.*E_CMF); //for one pbar
    xr = E1star / E_MAX_star;
    if(xr<0)return 0.0;
    if(xr>=1){
      // printf("BUGGGGGG\n");
      return 0.0;
    ...
    s = E_CMF*E_CMF;
    resultat *= invariant_multiplicity_pH_apX_MDGS_F12_CMF(xr,p1Tstar,s);
    E_CMF -= 2*E1star;
    E_CMF_SEUIL_PROTON = 4*MASSE_PROTON;
    if(E_CMF<E_CMF_SEUIL_PROTON){
      return 0;
    }
  }
  }
// if(resultat!=0)printf("resultat %e  B %e Ed3S_on_d3P_at_E_CMF %e Ed3S_on_d3P_at_E_available %e Ed3S_on_d3P_at_E_available_2 %e\n", resultat, B,Ed3S_on_d3P_at_E_CMF,Ed3S_on_d3P_at_E_available,Ed3S_on_d3P_at_E_available_2);
  return resultat; /* [cm^{2} GeV^{-2}] */
}
...
*/
  ...
  cos_theta = x3;
  sin_theta = sqrt(1. - pow(cos_theta,2));
  pL = cos_theta * P_pbar; //We evaluate the individual nucleus production CS at k_A/A
  pT = sin_theta * P_pbar;
  if(A_LA_CHARDONNET == _TRUE_)f3 = E_d3S_on_dP3_DBAR_LAB(E_proton,pL,pT,&E_CMF,&pLstar,&pTstar);
  else f3 = E_d3S_on_d3P_3HE_H_ON_H_LAB_MDGS_F12_coalescence(E_proton,pL,pT,P_coal,A_nuclei,Z_nuclei);
  ...
  for (i_theta=1;i_theta<=n_step;i_theta++)
  {
    ...
    pL = cos_theta * P_pbar;
    pT = sin_theta * P_pbar;
    if(A_LA_CHARDONNET == _TRUE_)f2 = E_d3S_on_dP3_DBAR_LAB(E_proton,pL,pT,&E_CMF,&pLstar,&pTstar);
    else f2 = E_d3S_on_d3P_3HE_H_ON_H_LAB_MDGS_F12_coalescence(E_proton,pL,pT,P_coal,A_nuclei,Z_nuclei);
...
- les antineutrons sont produits de manière plus abondantes dans les collisions d'où un facteur global (1. + 1.3) = 2.3
- Pour l'anti-hélium3, il faut tenir compte de l'anti-tritium produit à ALICE avec autant d'abondance que l'anti-hélium3
- Pas de facteur correctif par contre pour l'anti-deutérium et l'anti-hélium4
*/
  if(A_nuclei==1){resultat *= 2.3;} /* [cm^{2} GeV^{-1}] */
  if(A_nuclei==3){resultat *= 2.0;} /* [cm^{2} GeV^{-1}] */
	return resultat;
...
*/
  ...
  mt     = x3;
  pL     = sqrt(E_pbar*E_pbar - mt*mt);
  pT     = sqrt(mt*mt - pow(M_nuclei,2.0));
